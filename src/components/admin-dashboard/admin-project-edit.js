...
import './admin-project-edit.css';


const AdminProjectEditComponent = () => {
  ...
  const handleDelete = async (id) => {
    try {
      console.log('id',id)
      await axios.delete(`http://localhost:5000/api/v2/project/deleteprojects/${id}`);
  ...
  const handleSave = async (id) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/v2/ibex/updatecard/${id}`,
        formData,
      ...
      <div className="sponsors-container">
        <div className='sponsors-slider'>
          <div className='sponsors-slider-head'>
            ...
            <p className='sponsors-slider-para-head'>All Sponsors</p>
          </div>
          ...
          <div className='sponsors-image-container'>
            {visibleSponsors.map((item) => (
              <div key={item._id} className='sponsors-card'>
                <div className="sponsors-card-menu" onClick={() => toggleDropdown(item._id)}>
                  <FontAwesomeIcon icon={faEllipsisVertical} fontSize={'20px'} color='#dbb127' />
                  {showDropdown === item._id && (
                    <div className="sponsors-dropdown">
                ...
                {editMode === item._id ? (
                  ...
                  <div>
                    ...
                    />
                    <p className='sponsors-card-description'>{item.description}</p>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', width: '71%', color: 'white' }}>
                      <div className='sponsors-card-price'>
                        <p style={{ fontSize: '25px' }}>{'$' + item.projectcost}</p>
                        <p style={{ textDecoration: 'line-through' }}>{'$' + item.projecttype}</p>
                      </div>
...
export default AdminProjectEditComponent;